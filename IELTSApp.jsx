...
const LoginPage = ({ onLogin }) => {
  ...
  const handle = () => {
    ...
    setTimeout(() => {
      ...
      if (u) onLogin(u);
      else { setErr("Invalid credentials. Try student@ielts.com / 123"); setLoading(false); }
    }, 700);
  };
              ...
              <div style={{
                ...
                {recording && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--danger)", animation: "pulse-ring .8s infinite" }} />
                    <span style={{ color: "var(--danger)", fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>{fmtTime(recTime)}</span>
                  </div>
                )}
                <Btn onClick={handleRecord} variant={recording ? "danger" : "outline"}>
                  {recording ? "⏹ Stop Recording" : "⏺ Start Recording"}
                </Btn>
                <div style={{ fontSize: 12, color: "var(--muted)" }}>Or upload an audio file:</div>
                <input type="file" accept="audio/*" style={{ fontSize: 12, color: "var(--muted)" }} />
              </div>
              <Btn onClick={handleSubmit}>Submit Recording</Btn>
            </div>
          ) : (
            <div>
    ...
    setTimeout(() => {
      setGrammarResult({
        errors: [
          ...
          { text: "there house", suggestion: "their house", type: "grammar" },
        ],
        score: 82
      });
      setChecking(false);
        ...
        {grammarResult && (
          <div style={{ marginTop: 16, padding: 16, background: "var(--bg3)", borderRadius: 10 }}>
            <div style={{ fontWeight: 600, marginBottom: 10, display: "flex", justifyContent: "space-between" }}>
              <span>Grammar Check Results</span>
              <Badge label={`Score: ${grammarResult.score}/100`} color="success" />
            </div>
            {grammarResult.errors.map((e, i) => (
              <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6, fontSize: 13 }}>
                <span style={{ color: "var(--danger)" }}>✗ "{e.text}"</span>
                <span style={{ color: "var(--muted)" }}>→</span>
                <span style={{ color: "var(--success)" }}>"{e.suggestion}"</span>
                <Badge label={e.type} color="warn" />
              </div>
            ))}
          </div>
        )}
...
const ProgressPage = ({ user }) => {
  ...
  return (
    <div>
      ...
      {/* Skills */}
      <Card className="fade-up-3" style={{ marginBottom: 20 }}>
      ...
      {/* Streak */}
      <Card className="fade-up-4" style={{ background: "rgba(245,200,66,.06)", border: "1px solid rgba(245,200,66,.2)" }}>
